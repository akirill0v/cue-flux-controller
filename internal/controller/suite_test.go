...
func runInContext(registerControllers func(*testenv.Environment), run func() int) (code int) {
	var err error
	utilruntime.Must(cueinstancev1a1.AddToScheme(scheme.Scheme))
...
func TestMain(m *testing.M) {
	code := runInContext(func(testEnv *testenv.Environment) {
		controllerName := "cue-flux-controller"
		...
		if err := (reconciler).SetupWithManager(ctx, testEnv, CueInstanceReconcilerOptions{
			DependencyRequeueInterval: 2 * time.Second,
		}); err != nil {
			panic(fmt.Sprintf("Failed to start CueReconciler: %v", err))
		}
	}, m.Run)
...
func isReconcileRunning(k *cueinstancev1a1.CueInstance) bool {
	return conditions.IsReconciling(k) &&
		conditions.GetReason(k, meta.ReconcilingCondition) != meta.ProgressingWithRetryReason
}

func isReconcileSuccess(k *cueinstancev1a1.CueInstance) bool {
...
func isReconcileFailure(k *cueinstancev1a1.CueInstance) bool {
...
func logStatus(t *testing.T, k *cueinstancev1a1.CueInstance) {
...
func applyGitRepository(objKey client.ObjectKey, artifactName string, revision string) error {
	...
	opt := []client.PatchOption{
		...
		client.FieldOwner("cue-flux-controller"),
	}
