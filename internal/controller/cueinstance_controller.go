...
func (r *CueInstanceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, opts CueInstanceReconcilerOptions) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&cueinstancev1a1.CueInstance{}, builder.WithPredicates(
...
func (r *CueInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, retErr error) {
	...
	defer func() {
		...
		if conditions.IsReady(obj) {
			...
			r.event(obj, obj.Status.LastAppliedRevision, eventv1.EventSeverityInfo, msg,
				map[string]string{
					cueinstancev1a1.GroupVersion.Group + "/" + eventv1.MetaCommitStatusKey: eventv1.MetaCommitStatusUpdateValue,
				})
		}
	}()
	...
	if err != nil {
		conditions.MarkFalse(obj, meta.ReadyCondition, cueinstancev1a1.ArtifactFailedReason, err.Error())
	...
	if artifactSource.GetArtifact() == nil {
		...
		conditions.MarkFalse(obj, meta.ReadyCondition, cueinstancev1a1.ArtifactFailedReason, msg)
		log.Info(msg)
		return ctrl.Result{RequeueAfter: obj.GetRetryInterval()}, nil
	}
	...
	if len(obj.Spec.DependsOn) > 0 {
		if err := r.checkDependencies(ctx, obj, artifactSource); err != nil {
			conditions.MarkFalse(obj, meta.ReadyCondition, cueinstancev1a1.DependencyNotReadyReason, err.Error())
...
func (r *CueInstanceReconciler) reconcile(
	...
	// Create tmp dir.
	tmpDir, err := MkdirTempAbs("", fmt.Sprintf("cueinstance-%s", obj.Name))
	if err != nil {
	...
	if _, err := os.Stat(moduleRootPath); err != nil {
		err = fmt.Errorf("cueinstance root path not found: %w", err)
		conditions.MarkFalse(obj, meta.ReadyCondition, cueinstancev1a1.ArtifactFailedReason, err.Error())
		return err
	}

	// check build path exists
	dirPath, err := securejoin.SecureJoin(moduleRootPath, obj.Spec.Path)
	if err != nil {
		conditions.MarkFalse(obj, meta.ReadyCondition, cueinstancev1a1.ArtifactFailedReason, err.Error())
		return err
	}
	...
	// err := r.checkGates(ctx, revision, moduleRootPath, dirPath, &cueInstance)
	// if err != nil {
	// 	conditions.MarkFalse(obj, meta.ReadyCondition, cueinstancev1a1.BuildFailedReason, err.Error())
	// 	return err
	// }
	...
	conditions.MarkTrue(obj,
		meta.ReadyCondition,
		cueinstancev1a1.ReconciliationSucceededReason,
		fmt.Sprintf("Applied revision: %s", revision))

	return nil
}

func (r *CueInstanceReconciler) event(obj *cueinstancev1a1.CueInstance,
	...
	if revision != "" {
		metadata[cueinstancev1a1.GroupVersion.Group+"/revision"] = revision
	}
...
func (r *CueInstanceReconciler) finalizeStatus(ctx context.Context,
	obj *cueinstancev1a1.CueInstance,
	patcher *patch.SerialPatcher) error {
...
func (r *CueInstanceReconciler) patch(ctx context.Context,
	obj *cueinstancev1a1.CueInstance,
	...
	ownedConditions := []string{
		cueinstancev1a1.HealthyCondition,
		meta.ReadyCondition,
...
func (r *CueInstanceReconciler) getSource(ctx context.Context, obj *cueinstancev1a1.CueInstance) (sourcev1.Source, error) {
...
func (r *CueInstanceReconciler) checkDependencies(ctx context.Context,
	obj *cueinstancev1a1.CueInstance,
	...
	for _, d := range obj.Spec.DependsOn {
		...
		var c cueinstancev1a1.CueInstance
		err := r.Get(ctx, dName, &c)
		if err != nil {
			return fmt.Errorf("dependency '%s' not found: %w", dName, err)
		}

		if len(c.Status.Conditions) == 0 || c.Generation != c.Status.ObservedGeneration {
		...
		srcNamespace := c.Spec.SourceRef.Namespace
		if srcNamespace == "" {
			srcNamespace = c.GetNamespace()
		}
		...
		if c.Spec.SourceRef.Name == obj.Spec.SourceRef.Name &&
			srcNamespace == dSrcNamespace &&
			c.Spec.SourceRef.Kind == obj.Spec.SourceRef.Kind &&
			!source.GetArtifact().HasRevision(c.Status.LastAppliedRevision) {
...
func (r *CueInstanceReconciler) build(ctx context.Context,
	...
	shouldValidate := obj.Spec.Validate != nil
	...
	if len(obj.Spec.Exprs) > 0 {
		for _, e := range obj.Spec.Exprs {
			...
			data, err := cuemanageri.CueEncodeYAML(expr)
			if err != nil {
				return nil, err
			}

			if shouldValidate && obj.Spec.Validate.Type == "cue" {
				schema := value.LookupPath(cue.ParsePath(obj.Spec.Validate.Schema))
				schema.Unify(expr)
				if err := schema.Unify(expr).Validate(); err != nil {
					msg := fmt.Sprintf("cue expression validation failed: %s", err)
					switch obj.Spec.Validate.Mode {
					case cueinstancev1a1.FailPolicy:
						r.event(obj, revision, eventv1.EventSeverityInfo, msg, nil)
						return nil, fmt.Errorf(msg)
					case cueinstancev1a1.DropPolicy:
						r.event(obj, revision, eventv1.EventSeverityInfo, msg, nil)
						continue
					case cueinstancev1a1.AuditPolicy:
						r.event(obj, revision, eventv1.EventSeverityInfo, msg, nil)
						break
					case cueinstancev1a1.IgnorePolicy:
						log.Info(msg)
						break
					}
				}
		...
		data, err := cuemanageri.CueEncodeYAML(value)
		if err != nil {
			return nil, err
		}
		valid := false
		...
		if of.Encoding == "yaml" {
			...
			case cue.ListKind:
				...
				for l.Next() {
					...
					if shouldValidate && obj.Spec.Validate.Type == "yaml" {
						schema := value.LookupPath(cue.ParsePath(obj.Spec.Validate.Schema))
...
func (r *CueInstanceReconciler) checkHealth(ctx context.Context,
	manager *ssa.ResourceManager,
	...
	if len(obj.Spec.HealthChecks) == 0 && !obj.Spec.Wait {
		conditions.Delete(obj, cueinstancev1a1.HealthyCondition)
		return nil
	}
	...
	for _, o := range objects {
		if o.GroupKind.Kind == cueinstancev1a1.CueInstanceKind &&
			o.Name == obj.GetName() &&
	...
	wasHealthy := apimeta.IsStatusConditionTrue(obj.Status.Conditions, cueinstancev1a1.HealthyCondition)
	...
	conditions.MarkUnknown(obj, cueinstancev1a1.HealthyCondition, meta.ProgressingReason, message)
	if err := r.patch(ctx, obj, patcher); err != nil {
	...
	conditions.MarkTrue(obj, cueinstancev1a1.HealthyCondition, meta.SucceededReason, msg)
	if err := r.patch(ctx, obj, patcher); err != nil {
		return fmt.Errorf("unable to update the healthy status to progressing, error: %w", err)
	}
